...
/**
*******************************************************
Info:		Mini Project Part A RECEIVER
Author:		Avile Dlukwana and Kutlwisiso Belebesi
*******************************************************
Light-of-Things Concept
EEE3065/6S mini project part A receiver
******************************************************************************
  */
...
//global variables needed for the receiver side
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if(pressed)
	  {
		  ...
		  while(i < 12)
		  {
			  ...
			  sprintf(buffer, "bits = %lu", received_samples[i]);
			  debugPrintln(&huart2, buffer);
			  memset(buffer, 0, sizeof(buffer));
		  ...
		  while(i < 4)
		  {
			  ...
			  sprintf(buffer, "Checkbits = %lu", Samples[i]);
			  debugPrintln(&huart2, buffer);
			  memset(buffer, 0, sizeof(buffer));
			  HAL_Delay (1100); // wait for 500 ms
			  i++ ;
		...
		//check to see if the length is the same as transmitter checkpoint
		  sprintf(buffer, "Sample Value = %lu, checkpoint=%lu", samples_value, Trans_checkpoint);
		  debugPrintln(&huart2, buffer);
		  memset(buffer, 0, sizeof(buffer));
