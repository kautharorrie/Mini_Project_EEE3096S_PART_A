...
/**
*******************************************************
Info:		Mini Project Part A TRANSMITTER
Author:		Avile Dlukwana and Kutlwisiso Belebesi
*******************************************************
Light-of-Things Concept
EEE3065/6S mini project part A transmitter
******************************************************************************
  */
  ...
  while (1)
  {
	  if(adcValue)
	  {
		  ...
		  for(int j = 0 ; j < 12 ; j++  )
		  {
			  if(binaryNum[j] == 0)
			  {
				  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0,binaryNum[j] );
				  HAL_Delay (1000); // wait for 500 ms
		  ...
		  count = 0 ;
		  adcValue = 0 ;
		  ccrValue = ADCtoCRR(adcValue);
		  __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4, ccrValue);

		  decToBinary(checkpoint) ;
		  for(int j = 0 ; j < 4 ; j++  )
		  {
			 HAL_Delay (1000); // wait for 500 ms
			 HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0,binaryNum[j] );
